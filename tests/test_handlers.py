...
class TestFeedbackUpdateHandler:
    customer_user = None
    support_user = None
    ...
    def setup_class(cls):
        ...
        cls.handler: FeedbackUpdateHandler = cls.container.get(FeedbackUpdateHandler)
    ...
    def test_update_by_missing_user(self):
    ...
    def test_update_by_customer_user(self):
    ...
    def test_update_by_missing_feedback(self):
    ...
    def test_update_status(self):
        modified_feedback = self.handler.execute(
            user_id=self.support_user.user_id,
            feedback_id=self.customer_user_feedback.feedback_id,
            status="developing",
        )
        assert modified_feedback.status == "developing"
        assert modified_feedback.support_comment is None

    def test_update_support_comment(self):
        modified_feedback = self.handler.execute(
            user_id=self.support_user.user_id,
            feedback_id=self.customer_user_feedback.feedback_id,
            support_comment="対応検討を依頼したよ",
        )

        assert isinstance(modified_feedback.support_comment, str)
        assert modified_feedback.status == "new"
